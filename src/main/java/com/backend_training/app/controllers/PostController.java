...
public class PostController {
    ...
    @Autowired
    public PostController(RateLimiterService rateLimiterService) {
        this.rateLimiterService = rateLimiterService;
    }

    @GetMapping
    public ResponseEntity<PostResponse> getPosts(@RequestParam(defaultValue = "10") int limit, @RequestParam(required = false) String cursor) throws Exception {
        return ResponseEntity.ok(postService.fetchPosts(cursor, limit));
    }

    @PostMapping
    public ResponseEntity<?> createPost(@RequestBody Post post, @RequestAttribute("userID") String userID) {
